...
all: build/hard_cap.so build/total_supply_cell_lock

all-via-docker: ${PROTOCOL_HEADER}
	docker run --rm -v `pwd`:/code ${BUILDER_DOCKER} bash -c "cd /code && make"

ALL_C_SOURCE := $(wildcard c/hard_cap.c c/total_supply_cell_lock.c)
...
build/total_supply_cell_lock: c/total_supply_cell_lock.c
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $<
	$(OBJCOPY) --only-keep-debug $@ $@.debug
	$(OBJCOPY) --strip-debug --strip-all $@
